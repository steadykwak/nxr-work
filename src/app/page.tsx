...
export default async function Home() {
  ...
  const isImported = Boolean(
    connection?.initial_imported_at || connection?.last_sheet_sync_at,
  );
  ...
  if (connection && !isImported) {
    try {
      ...
      const { data: refreshedTasks } = await adminClient()
        .from('tasks')
        .select('*')
        .eq('user_id', user.id)
        .order('source_due_date', { ascending: true });
  ...
  return (
    <Dashboard
      ...
      initialImportedAt={
        connection?.initial_imported_at ??
        connection?.last_sheet_sync_at ??
        undefined
      }
