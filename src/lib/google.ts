import { isLunchEvent } from './today';
...
export async function accessToken(userId: string) {
  ...
  if (!response.ok)
    throw new Error('Google 토큰 갱신에 실패했습니다. 다시 연결해 주세요.');
...
export async function calendarEvents(token: string): Promise<CalendarEvent[]> {
  ...
  if (!response.ok)
    throw new Error(`Google Calendar 읽기 실패 (${response.status})`);
  ...
  return (data.items ?? [])
    ...
    .filter((event) => !isLunchEvent(event));
}
