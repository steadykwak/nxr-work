...
3. Authentication → Providers에서 이메일 OTP를 활성화합니다. URL Configuration의 Site URL에 `http://localhost:3000`, Redirect URLs에 `http://localhost:3000/auth/callback`을 추가합니다.
4. Project URL, publishable/anon key, secret key 또는 기존 service role key를 프로젝트 설정에서 확인합니다. secret/service role key는 서버 환경 변수에만 넣습니다.
